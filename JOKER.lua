...
-----------------------------------------------------------------------------------------------
--  BY JOKER TEAM       ---       KEKO JSON              ---          BY  JOKER TEAM        --
-----------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------
--  BY JOKER TEAM       ---      End KEKO JSON              ---          BY JOKER TEAM       --
-----------------------------------------------------------------------------------------------
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "🦁┇JOKER  TEAM")..'', url=''..(channel_user_ts or 't.me/JOKER TEAM')..''},
  },
  }
...
files_JOKER 2 = files_JOKER 2..'{'..files_JOKER [i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑️┇جميع الملفات : \n '..keko..'\n ---------------------- \n\n✔️┇الملفات المفعله \n'..files_JOKER 2..'', 1, 'html')
end
...
local JOKER e = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. JOKER e .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_JOKER /'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_JOKER /'..data.message_.content_.document_.file_name_)
...
local files_JOKER  = database:smembers("files"..bot_id)
for i=1,#files_JOKER  do
local JOKER ee = dofile("files_JOKER /"..files_JOKER [i])
local kt = JOKER ee.keko_JOKER (data)
if kt == 'end' then
return false
end
end
...
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
 ...
 f:close()
 local JOKER e = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. JOKER e .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'JOKER .txt' .. '"'
 io.popen(curl)
...
os.execute('wget https://raw.githubusercontent.com/moodlIMyIl/JOKER /master/JOKER .lua')
 return false end
...
local nakeko = redis:get('nakeko'..bot_id)
  ...
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkeko or 9647707641864), (nakeko or "JOKER  TEAM"), "", bot_id)
end
 end
  for k,v in pairs(sudo_users) do
...
local keko222 = 'https://JOKER .tk/JOKER /ch.php?id='..msg.sender_user_id_..''
...
◀┇<a href="https://telegram.me/llJOKERll">قناه شروحات سورس، 🦁" </a>

🔎┇<a href="https://github.com/moodlIMyIl/JOKER ">رابط Github Cli (الرقم)،⚜️ </a>

🔎┇<a href="https://github.com/moodlIMyIl/JOKER ">رابط Github Api (التوكن)،⚜️ </a>
